...
void Frame::SetNormalMap(const float* normal)
{
  if (NormalMap)
    delete [] NormalMap;
  NormalMap = new float [Size[0] * Size[1] * 3];
  memcpy(NormalMap, normal, sizeof(float) * Size[0] * Size[1] * 3);
}
...
bool Frame::Write()
{
  ...
  for (unsigned int i = 0; i < ScalarMaps.size(); ++i)
  {
    char istr[5];
    sprintf(istr, "%d", i);
    PNGWriter scalar_writer(FileName + "_scalar_" + istr, Size[0], Size[1]);
